...
# This is a closure function, where the parameters are available to the inner functions, and the returned function (namely rename()) will get called after build_rename gets called.
# The call order will be as follow:
#   var_name = build_rename(i3, app_icons, args) #Note that you will need to assign this function call to a variable to be able to access the inner function at the next step
#   var_name(i3,args) #Note that python know which function to execute when calling that variable since inside your closure (buld_rename()), you returned the function (rename()), so when you execute your variable, the rename function will run.
# Closure is just a way to hide function from misused by external users and provide a single function/interface for users.
def build_rename(i3, app_icons, args):
    ...
    def get_icon_or_name(leaf, length):
        # Loop every attribute for a leaf object and see if that attribute exist
        for identifier in ('name', 'window_title', 'window_instance', 'window_class'):
            # getattr is equal to dot notation, but provide default value as 3rd parameter and allow dynamic attribute name (as variable)
            name = getattr(leaf, identifier, None)
            if name is None:
                continue
            # get keys (application name) from app_icons.JSON
            for name_re in app_icons.keys():
                # regex compare while ignoring case AND icon is in fa_icons
                if re.match(name_re, name, re.IGNORECASE) \
                   and app_icons[name_re] in icons:
                    return icons[app_icons[name_re]]
        # If name is not None but does not find icon
        if name:
    ...
    def rename(i3, e):
        # grabbing the i3 workspace containers
        ...
        # grabbed the visible worksapces
        visible = [workdict.name for workdict in workdicts if workdict.visible]
        visworkspaces = []
        # grabbed the focused worksapces or None if it found none
        # there will only be 1 focus workspace but since we are enumerating an array, we need to use [0] to get that value. [None] is just to comply with the array enumation, eventually it will be converted to None b/c of [0]
        focus = ([workdict.name for workdict in workdicts if workdict.focused] or [None])[0]
        focusname = None

        commands = []
        # Leaves are applications within a workspace
        for workspace in workspaces:
            # names should contain icons from get_icon_or_name function
            names = [get_icon_or_name(leaf, length)
                     for leaf in workspace.leaves()]
            # remove duplicate, uniq is an argument from CLI
            if uniq:
                seen = set()
                names = [x for x in names if x not in seen and not seen.add(x)]
            # join icons or names with delim symbol (default to | )
...
def _get_i3_dir():
    # standard i3-config directories (go in order)
...
def _get_app_icons(config_path=None):
    ...
    # If CLI argument provide a path
    if config_path:
        # File not found
        if not os.path.isfile(config_path):
            raise SystemExit("Specified app-icon config path '{}' does not exist".format(config_path))
    # If CLI argument does not provide a path
    else:
        # This will return i3 config path and append app-icons.josn filename after the path
        config_path = os.path.join(_get_i3_dir(), "app-icons.json")

    # If app-icons.json exist in _get_i3_dir directory
    if os.path.isfile(config_path):
        with open(config_path) as f:
            app_icons = json.load(f)
        # normalise app-names to lower case
        return {k.lower(): v for k, v in app_icons.items()}
    # If app-icons.json does not exist in _get_i3_dir directory
    else:
...
def main():
    ...
    # __doc__ is the string from line 2
    ...
    # app_icons JSON file might not exist, use default if cannot find any
    # return type: dict
    app_icons = _get_app_icons(args.config_path)

    # check for missing/wrongly named icons
    for app, icon_name in app_icons.items():
        # Check against fa-icons
